...
def posterior_decoding(model, eval_corpus, show_cross_entropy=False):
    if show_cross_entropy:
        model_cross_entropy(model, eval_corpus)
    return tagger_error_rate(posterior_tagger(model, eval_corpus), eval_corpus)

def posterior_tagger(model: HiddenMarkovModel, eval_corpus: TaggedCorpus):
    def tagger(input:Sentence) -> Sentence:
